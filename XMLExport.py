...
class XMLExport(Dialogs.GenForm):
    ...
    def __init__(self,parent,id,title):
        ...
        # Add a Text block for the instructions text
        exportTxtCtrl = wx.StaticText(self.panel, -1, exportTxt)
        mainSizer.Add(exportTxtCtrl, 0, wx.EXPAND | wx.ALL, 5)
        mainSizer.Add((1, 15))

        h2 = wx.BoxSizer(wx.HORIZONTAL)
        # Add a choice for the user between full and codebook export
        contentTxtCtrl = wx.StaticText(self.panel, -1, _("Data to Export"))
        h2.Add(contentTxtCtrl, 0, wx.ALL | wx.ALIGN_CENTER_VERTICAL, 5)
        self.contentCtrl = wx.Choice(self.panel, -1, choices=[_('Full Database'), _('Codebook Only')])
        # default to full export
        self.contentCtrl.SetSelection(0)
        h2.Add(self.contentCtrl, 1, wx.EXPAND | wx.ALL | wx.ALIGN_CENTER_VERTICAL, 5)
        mainSizer.Add(h2, 0, wx.EXPAND)
        ...
        self.XMLFile = self.new_edit_box(_("Filename"), h3, '')
        ...
        h3.Add(browse, 0, wx.ALIGN_BOTTOM)
        h3.Add((5, 1))
        # If Mac ...
    ...
    def OnBrowse(self, evt):
        ...
        fs = wx.FileDialog(self, _("Select a file for export"),
                        TransanaGlobal.configData.videoPath,
                        "",
                        fileTypesString, 
                        wx.FD_SAVE)
        # Default to Transana-XML format
        fs.SetFilterIndex(1)
    ...
    def CalcPercent(self, num):
        ...
        # If we have a Transana-XML file ...
            ...
            # ... we have 15 categories for export
            numCategories = 15.0
        # Return the appropriate integer representation of the progress percentage
    ...
    def WriteXMLDTD(self, f):
        ...
        if self.contentCtrl.GetSelection() == 0:
            ...
            f.write('  <!ELEMENT Transana (#PCDATA|KeywordFile)*>\n')
        f.write(']>\n')
        f.write('\n')
...
##                    dbCursor.close()
##
##                progress.Update(self.CalcPercent(4), _('Writing Collection Records'))
##                if db != None:
##                    dbCursor = db.cursor()
##                    SQLText = 'SELECT CollectNum, CollectID, ParentCollectNum, CollectComment, CollectOwner, DefaultKeywordGroup FROM Collections2'
##                    dbCursor.execute(SQLText)
...
##                    dbCursor.close()
##
##                progress.Update(self.CalcPercent(5), _('Writing Quote Records'))
##                if db != None:
##                    dbCursor = db.cursor()
##                    SQLText = 'SELECT QuoteNum, QuoteID, CollectNum, SourceDocumentNum, SortOrder, Comment, XMLText FROM Quotes2'
##                    dbCursor.execute(SQLText)
...
##                    dbCursor.close()
##
##                progress.Update(self.CalcPercent(6), _('Writing Quote Position Records'))
##                if db != None:
##                    dbCursor = db.cursor()
##                    SQLText = 'SELECT QuoteNum, DocumentNum, StartChar, EndChar FROM QuotePositions2'
##                    dbCursor.execute(SQLText)
...
##                    dbCursor.close()
##
##                progress.Update(self.CalcPercent(7), _('Writing Clip Records'))
##                if db != None:
##                    dbCursor = db.cursor()
##                    SQLText = 'SELECT ClipNum, ClipID, CollectNum, EpisodeNum, MediaFile, ClipStart, ClipStop, ClipOffset, Audio, '
##                    SQLText += 'ClipComment, SortOrder FROM Clips2'
...
##                    dbCursor.close()
##
##                progress.Update(self.CalcPercent(8), _('Writing Transcript Records  (This will seem slow because of the size of the Transcript Records.)'))
##                if db != None:
##                    dbCursor = db.cursor()
##                    dbCursor2 = db.cursor()
##                    # Load Transcript Information WITHOUT RTFText content, which requires too much memory in some large database
...
##                    dbCursor.close()
##
##                progress.Update(self.CalcPercent(9), _('Writing Snapshot Records'))
##                if db != None:
##                    dbCursor = db.cursor()
##                    SQLText = 'SELECT SnapshotNum, SnapshotID, CollectNum, ImageFile, ImageScale, ImageCoordsX, ImageCoordsY, '
##                    SQLText += 'ImageSizeW, ImageSizeH, EpisodeNum, TranscriptNum, SnapshotTimeCode, SnapshotDuration, '
...
##                    dbCursor.close()
            ...
            # This block is executed for both "Full Database" and "Codebook Only" exports
...
##            if self.contentCtrl.GetSelection() == 0:
##
##                progress.Update(self.CalcPercent(11), _('Writing Clip Keyword Records'))
##                if db != None:
##                    dbCursor = db.cursor()
##                    SQLText = 'SELECT EpisodeNum, DocumentNum, ClipNum, QuoteNum, SnapshotNum, KeywordGroup, Keyword, Example FROM ClipKeywords2'
##                    dbCursor.execute(SQLText)
...
##                    dbCursor.close()
##
##                progress.Update(self.CalcPercent(12), _('Writing Snapshot Keywords Records'))
##                if db != None:
##                    dbCursor = db.cursor()
##                    SQLText = 'SELECT SnapshotNum, KeywordGroup, Keyword, x1, y1, x2, y2, visible FROM SnapshotKeywords2'
##                    dbCursor.execute(SQLText)
...
##                    dbCursor.close()
##
##                progress.Update(self.CalcPercent(13), _('Writing Snapshot Coding Style Records'))
##                if db != None:
##                    dbCursor = db.cursor()
##                    SQLText = 'SELECT SnapshotNum, KeywordGroup, Keyword, DrawMode, LineColorName, LineColorDef, LineWidth, LineStyle '
##                    SQLText += 'FROM SnapshotKeywordStyles2'
...
##                    dbCursor.close()
##
##                progress.Update(self.CalcPercent(14), _('Writing Note Records'))
##                if db != None:
##                    dbCursor = db.cursor()
##                    SQLText = 'SELECT NoteNum, NoteID, SeriesNum, EpisodeNum, CollectNum, ClipNum, SnapshotNum, DocumentNum, '
##                    SQLText += 'QuoteNum, TranscriptNum, NoteTaker, NoteText FROM Notes2'
...
##        f.write('  <!ELEMENT QDA-XMLVersion (#PCDATA)>\n')
##        f.write('  <!ELEMENT SeriesFile (Series)*>\n')
##        f.write('\n')
##        f.write('  <!ELEMENT Num (#PCDATA)>\n')
##        f.write('  <!ELEMENT ID (#PCDATA)>\n')
