                  ...
                  ((dataType == 'NoteText') and (lineUpper.lstrip() == '</NOTETEXT>')):
                   ...
                   # once the data is captured.
                       ...
                       if (self.XMLVersionNumber in ['1.1', '1.2', '1.3']):
                           ...
                           currentObj.definition = currentObj.definition + self.UnEscape(line.decode('utf8'))  # (self.importEncoding)
...
#           print "XMLImport - Begin Transaction"
       ...
       # We need to track the number of lines read and processed from the input file.
       lineCount = 0
       # Object Counting is mostly for debugging
       objCountNumber = 1
...
##           collectionsToUpdate = []
           ...
           # Create a dictionary to track the GUIDs for Codes
           keywordsDict = {}
           ...
           for line in f:
               # ... increment the line counter
               lineCount += 1
               # If we DON'T have description Text ...
               if not (dataType in  ['description']):
                   # ... we can't just use strip() here.  Strings ending with the a with a grave (Alt-133) get corrupted!
                   line = Misc.unistrip(line)
               # If we have RTF Text or Note Text, we don't want to strip leading white space ...
               else:
                   # ... so we do unicode stripping only of the RIGHT side here.  Left spaces could be meaningful.
                   line = Misc.unistrip(line, left=False)
...
##               if ((line != '') and not (dataType in ['XMLText', 'RTFText']) and (line[0] == '<')) or \
##                  ((dataType == 'NoteText') and (lineUpper.lstrip() == '</NOTETEXT>')):
               if ((line != '') and not (dataType in ['description']) and (line.lstrip()[0] == '<')):
...
##                       skipValue = MainHeads[lineUpper]['skipValue']
                   ...
                   # (WAS ELIF under MainHeads)
                   # If our line is one of our DataTypes ...
                   if lineUpper.lstrip() in DataTypes.keys():
                       # ... update the dataType.
                       dataType = DataTypes[lineUpper.lstrip()]
...
##                       dataType = None
                   ...
                   # If we are defining a CODE ...
                   elif lineUpper == '<CODE>':
                       # ... we'll use a dictionary to import object data, as it will not conform to our Keyword object (yet).
                       currentObj = {}
                       # The type of the current object is Keyword
                       objectType = 'Keyword'
                       # The data type (element within the object) is not yet defined
                       dataType = None
...
##                        lineUpper == '</FILTER>':
                       ...
                       # The Data Type (within the object) is undefined.
                       dataType = None
...
#                           print
#                           print "Need to save currentObj to DB or LISTobj here"
#                           print
...
##                               currentObj.db_save()
                           ...
                           # SHOULD BE ELIF
                           # If we are saving a Keyword ...
                           if objectType == 'Keyword':

                               if DEBUG:
                                   print "Keyword Object"
                                   print currentObj
                                   print
                                   
                                   objCountNumber += 1

                               # Let's save it to the keywords dictionary. We'll put it in the database later
                               keywordsDict[int(currentObj['idNum'])] = currentObj
...
##                               errordlg.Destroy()
                       ...
                       # We just saved an object. reset the object and data types.
                       currentObj = None
                       objectType = None

               # For non-XML commands ...
               else:

                   # Let's start with a crude handler for Dictionary objects
                   # If we know the object type and the datatype...
                   # (for Keywords, definition is excluded)
                   if (objectType in ['Keyword'] and \
                       dataType in ['colorDef', 'idNum', 'isCodable', 'name', 'parent', 'type']):
                       # ... assign the datatype to the current object dictinary using datatype as the key
                       currentObj[dataType] = self.ProcessLine(line)
...
##                   # this with fewer preliminary "if" checks!
                   ...
                   # If we have a closing XML tag for a multi-line item, we have to catch that here ...
                   if lineUpper.lstrip() in ['</DESCRIPTION>']:
                       # ... clear out the DataType
                       dataType = None
                   # If we are inside a DESCRIPTION ...
                   elif dataType in ['description']:
                       # ... if we are in a Keyword ...
                       if objectType in ['Keyword']:
                           # ... if we don't already have a DESCRIPTION field in the Keyword CurrentObj ...
                           if not currentObj.has_key(dataType):
                               # ... create a Description item within the dictionary
                               currentObj[dataType] = self.ProcessLine(line).lstrip()
                           # If the dictionary HAS a DESCRIPTION key ...
                           else:
                               # ... add the line to the existing data
...
##                   dbCursor2.close()
               ...
               for key in keywordsDict.keys():
                   ...
                   if DEBUG:
                       print '  ', key, keywordsDict[key]

                   # If we have not yet ruled Transana Format out ...
                   if keywordsInTransanaFormat:
                       
                       # Keyword Groups are not codable
                       if (keywordsDict[key]['parent'] == '0') and (keywordsDict[key]['isCodable'] == '1'):
                           keywordsInTransanaFormat = False

                           if DEBUG:
                               print ' -- A', keywordsDict[key]['name'], keywordsDict[key]['parent'], keywordsDict[key]['isCodable']
                           
                       # Transana allows exactly two levels of keyword hierarchy
                       if (keywordsDict[key]['parent'] != '0') and (keywordsDict[int(keywordsDict[key]['parent'])]['parent'] != '0'):
                           keywordsInTransanaFormat = False

                           if DEBUG:
                               print ' -- B', keywordsDict[key]['name'], keywordsDict[key]['parent'], keywordsDict[int(keywordsDict[key]['parent'])]['parent']
                           
                       # Second Level must be codable
                       if (keywordsDict[key]['parent'] != '0') and (keywordsDict[key]['isCodable'] == '0'):
                           keywordsInTransanaFormat = False

                           if DEBUG:
                               print ' -- C', keywordsDict[key]['name'], keywordsDict[key]['parent'], keywordsDict[key]['isCodable']
               ...
               if keywordsInTransanaFormat:
                   ...
                   for key in keywordsDict.keys():
                        ...
                        if (keywordsDict[key]['parent'] != '0') and (keywordsDict[key]['isCodable'] == '1'):
                            ...
                            keywordObj.keywordGroup = keywordsDict[int(keywordsDict[key]['parent'])]['name']
                            # ... the Name is the Keyword
                            keywordObj.keyword = keywordsDict[key]['name']
                            # If there's a description ...
                            if keywordsDict[key].has_key('description'):
                                # ... that's the definition
                                keywordObj.definition = keywordsDict[key]['description']
                            # if there's a Color definition ...
                            if keywordsDict[key].has_key('colorDef'):
                                # ... then we have a color definition
                                keywordObj.lineColorDef = keywordsDict[key]['colorDef']
               ...
               # If we have Codes in Non-Transana Format ...
                    ...
                    for key in keywordsDict.keys():
                        ...
                        if (keywordsDict[key]['isCodable'] == '1'):
                            # Create a Keyword Object
                            keywordObj = Keyword.Keyword()
                            # Declare "Imported" as the Keyword Group
                            keywordObj.keywordGroup = _('Imported')
                            ...
                            while (keywordsDict[tmpKey]['parent'] != '0'):
                                # Remember the parent as the new key 
                                tmpKey = int(keywordsDict[tmpKey]['parent'])
                                # Add the Name to the FRONT of the keyword value, colons separating levels
                                keywordObj.keyword = keywordsDict[tmpKey]['name'] + ' : ' + keywordObj.keyword
...
##               print
    ...
    def OnBrowse(self, evt):
        ...
        # If not ...
            ...
            # ... then skip it
            fileTypes = _("Transana-XML Files (*.tra)|*.tra|XML Files (*.xml)|*.xml|All files (*.*)|*.*")
